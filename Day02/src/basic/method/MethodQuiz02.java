...
public class MethodQuiz02 {
	...
	static void push(String newFood){
		...
		String[] tmp = new String[foods.length+1];
		
			for(int i = 0;i<foods.length;i++) {
			tmp[i] = foods[i];
			}
			
			tmp[foods.length] = newFood;
	...
	public static void main(String[] args) {
		...
		//김치찌개의 인덱스를 알아내세요.
		System.out.println(indexOf("김치찌개"));           //만약에 찾지 못하는 음식이면
		//짜장면의 인덱스를 알아내세요. -> -1을 리턴하시면 됩니다.
		System.out.println(indexOf("짜장면")); 
		
		System.out.println(include("파스타"));
		//김치찌개의 인덱스를 알아내세요.
		System.out.println(include("김치찌개"));           //만약에 찾지 못하는 음식이면
		//짜장면의 인덱스를 알아내세요. -> -1을 리턴하시면 됩니다.
		System.out.println(include("짜장면")); 
