 ...
 *  
 *  2. 매개변수는 여러 개 존재할 수 있지만,하지만 반환값은 오직 한개만
 * 		존재해야 한다.
 *  
 ...
 *     반환값을 다른 변수에 대입할 수도 있고, 다른 메서드의 매개값으로도 처리될
 *     수 있습니다.
 *  
 *  6. 모든 메서드가 반환값이 있는것은 아닙니다. 메서드 실행 후에
 *  반환할 값이 딱히 없다면 return을 생략해도 됩니다.
 *  
 *  7.이 때는 반환유형을 메서드를 선언할 때 비워두지 마시고 void라는 키워드를
 *  작성해 주셔야 합니다.
 *  
 *  8. 모든 메서드는 return을 만나면 강제로 메서드가 종료된다.
 *  따라서 조건 없이 return문 아래에 코드를 작성할 수 없다.
 *  void 메서드에서는 return 키워드를 탈출 기능으로 사용할 수 있다.
 *  void 타입이 아닐때 return을 탈출 키워드로 사용할 수 없다. 반환값이 있어야 한다.
 *  
 */
...
//		
	...
	static double[] calcArrayTotal() {
		int[] a =new int[] {57,89,78,91,93,47};
		int total01 = 0;
		...
		return new double[] {total01,total01/(double)a.length};
	}
	...
	public static void main(String[] args) {
		...
		double[] soo = calcArrayTotal();
		System.out.printf("합계: %d 평균:%.2f",(int)soo[0],soo[1]);
		 ...
		 * 리턴하는 메서드입니다.
		 * 
		 * 3. 2번에 정의한 조건대로 메서드를 선언하시고,실제로 main에서 호출
		 * 해서 합계와 평균(소수점 둘째 자리까지)을 출력해 주세요.
		 * {57,89,78,91,93,47}
		 * 
		 */
...
//		System.out.println(multi(10,7)); (x)
		...
		multi(add(3,4),add(4,7)); //add는 리턴값이 있다.
//		add(multi(4,4),multi(5,6)); void는 리턴값이 없다.
		/*
		 * 반환값이 없는 void 타입의 메서드는 값이 들어오지 않기 때문에
