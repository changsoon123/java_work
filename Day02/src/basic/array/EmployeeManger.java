package basic.array;

import java.util.Arrays;
import java.util.Scanner;

public class EmployeeManger {

	public static void main(String[] args) {
		
		
		Scanner sc = new Scanner(System.in);
		//사원의 정보: 사번,이름, 나이, 부서명
		String[] userNums = new String[100];
		String[] names = new String[100];
		int[] ages = new int[100];
		String[] deps = new String[100];
		
		//실제로 입력된 데이터(사원의 정보)의 개수를 체크하는 변수.
		int count =0;
		
		
		sop:while(true) {
			System.out.println("\n========= 사원 관리 프로그램 ==========");
			System.out.printf(" count 값: %d\n",count);
			
			System.out.println("\n==================================");
			System.out.println("#1. 사원 정보 신규 등록");
			System.out.println("#2. 모든 정보 보기");
			System.out.println("#3. 사원 정보 검색");
			System.out.println("#4. 사원 정보 수정");
			System.out.println("#5. 사원 정보 삭제");
			System.out.println("#6. 프로그램 종료");
			System.out.println("> ");
			
			int menu = sc.nextInt();
			
			if(menu==1) {
				//사원 정보 4가지를 입력받아 각 배열에 저장하는 코드를 작성.
                //사번은 중복되면 안됩니다.
                //(무한루프를 구현해서 사번 중복이 발생하면 다시 입력받기.이름 중복은 허용)
			 
				outer : for(int i=count;i<100;i++) {
					System.out.print("신규 사번을 입력해주세요 : ");
					String inputNum = sc.next();
					for(int j=0;j<100;j++) {
						if(inputNum.equals(userNums[j])){
							System.out.println("사번이 중복되었습니다. 다시입력해주세요");
							i--;
							break;
						}else if (j==99 && !(inputNum.equals(userNums[j]))) {
							userNums[i] = inputNum;
							System.out.print("신규 이름을 입력해주세요 : ");
							String inputName = sc.next();
							names[i] = inputName;
							System.out.print("신규 나이을 입력해주세요 : ");
							int inputAge = sc.nextInt();
							ages[i] = inputAge;
							System.out.print("신규 부서명을 입력해주세요 : ");
							String inputdep = sc.next();
							deps[i] = inputdep;
							System.out.println("--------------");
							System.out.println("신규등록 되었습니다.");
							System.out.println("--------------");
							System.out.printf("사번 : %s\n",userNums[i]);
							System.out.printf("이름 : %s\n",names[i]);
							System.out.printf("나이 : %d\n",ages[i]);
							System.out.printf("부서 : %s\n",deps[i]);
							System.out.println();
							count++;
							break outer;
						}
					}
				}
				
				
			} else if(menu == 2) {
				//각 배열을 반복문을 통해 저장된 데이터까지만 출력하도록 작성. (count)
                //만약 사용자가 사원 등록을 한 명도 하지 않았다면
                //"등록된 사원 정보가 없습니다." 를 출력하시면 됩니다.
				
				if(count!=0) {
				for(int i=0;i<100;i++) {
					if(userNums[i]==null) {
						break;
					}
					System.out.printf("사번 : %s\n",userNums[i]);
					System.out.printf("이름 : %s\n",names[i]);
					System.out.printf("나이 : %d\n",ages[i]);
					System.out.printf("부서 : %s\n",deps[i]);
				}
				} else {
					
					System.out.println("등록한 정보가 없습니다. \n");
					
				}
				
			} else if(menu == 3) {
				//입력한 사번에 일치하는 사원의 4가지 정보를 모두 출력하세요.
                //입력한 사번이 존재하지 않는다면 "조회하신 사원의 정보가 없습니다." 를 출력.
				System.out.println("사원 번호를 입력하세요");
				System.out.println("종료는 0");
				String soo = sc.next();
					sol:for(int i = 0;i<100;i++) {
						if(soo.equals(userNums[i])){
					System.out.printf("사번 : %s\n",userNums[i]);
					System.out.printf("이름 : %s\n",names[i]);
					System.out.printf("나이 : %d\n",ages[i]);
					System.out.printf("부서 : %s\n",deps[i]);
						break;
						} else if(i==99 && !(soo.equals(userNums[i]))) {
							System.out.println("조회하신 사원의 정보가 없습니다");
							break sol;
						} else if(soo.equals("0")) {
							System.out.println("나갑니다");
							break sol;
						}
					}
				
				
				
			} else if(menu == 4) {
				//해당 사번과 일치하는 사원의 나이와 부서를 선택적으로 변경할 수 있도록
                //프로그램을 구성하시고, 나이와 부서를 수정하는 코드를 작성하세요.
                //사번 입력 -> 존재하는 사번이 맞다면 -> [1. 나이변경 | 2. 부서변경 | 3.취소]
                //사번이 존재하지 않는다면 없다고 얘기해 주세요.
				
				System.out.println("수정할 사원 번호를 입력하세요");
				System.out.println("종료는 0");
				String soo1 = sc.next();
				
					soi:for(int i = 0;i<100;i++) {
						if(soo1.equals(userNums[i])) {
							System.out.println("1. 나이변경 | 2. 부서변경 | 3.취소");
							String soo2 = sc.next();
							if(soo2.equals("1")) {
								System.out.println("변경할 나이를 입력해주세요. ");
								int soo3 = sc.nextInt();
								ages[i] = soo3;
								System.out.printf("사원:%d, 나이:%d, 이름:%d, 부서:%d로 변경됬습니다. ",userNums[i],names[i],ages[i],deps[i]);
								break;
								
							} else if(soo2.equals("2")) {
								System.out.println("변경할 부서를 입력해주세요. ");
								String soo4 = sc.next();
								deps[i] = soo4;
								System.out.printf("사원:%d, 나이:%d, 이름:%d, 부서:%d로 변경됬습니다. ",userNums[i],names[i],ages[i],deps[i]);
								break;
								
							}	
							 else if(soo2.equals("3")) {
								 System.out.println("나갑니다. ");
								 break;
							 } 
						}	 
						 else if(i==99 && !(soo1.equals(userNums[i]))) {
							System.out.println("존재하는 사번이 없습니다. ");
							break soi;
							
						}
						
						else if(soo1.equals("0")) {
							break soi;
						}
					}
				
				
			} else if(menu == 5) {
				//사번을 입력받아서
				//해당 사번과 일치하는 사원의 4가지 정보를 각 배열에서 모두 삭제하고
                //삭제하기 전에 "정말 삭제하시겠습니까? [Y/N]" 를 입력받아서
                //y를 눌렀을 시에 삭제가 되도록 코드를 구성하세요.
                //배열의 크기는 줄이지 않겠습니다. 삭제할 인덱스를 기준으로 뒤에 있는 값들을
                //앞으로 한칸씩 땡기신 다음 count를 하나 내려주시면 됩니다.
				
				System.out.println("삭제할 사번을 입력해주세요. ");
				String ssc = sc.next();
				
			hoho:for(int i=0;i<100;i++) {
				if(ssc.equals(userNums[i])) {
					System.out.println("정말 삭제하시겠습니까?? [Y/N] ");
					String sool = sc.next();
					if(sool.equals("Y") || sool.equals("y")) {
						for(int j=i;j<100;j++) {
							if(j==99 && !(userNums[j]==null)) {
								
								userNums[99]=null;
								names[99]=null;
								ages[99]=(Integer) null;
								deps[99]=null;
								count--;
								break hoho;
							}
							
					userNums[j]=userNums[j+1];
					names[j]=names[j+1];
					ages[j]=ages[j+1];
					deps[j]=deps[j+1];
					if(j==98 && userNums[j]==null) {
					count--;
					break;
					}
						}
					} else {
						break;
					}
				
				} else if(i==99 && !(ssc.equals(userNums[i]))) {
					System.out.println("존재하는 사번이 없습니다. ");
					break ;
					
				}
				
				else if(ssc.equals("0")) {
					break hoho;
				}
				
				
				
			}
				
				
			
			
			}
			
			
		} //end while true
		
		
		
	} //end main

}
